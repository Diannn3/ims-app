...
const route: RouteResult = {
  nodeIds: ['start', 'g1', 'g2', 'g-stairs', '2-stairs', '2a', 'dest'],
  totalCost: 1
};

describe('route instructions', () => {
  it('creates start, turns, floor changes, and arrival guidance', () => {
    const steps = buildRouteInstructions(graph, route, {
      destinationLabel: 'MB 209',
  ...
  it('keeps every instruction associated with route node ids for camera synchronization', () => {
    const steps = buildRouteInstructions(graph, route);
    expect(steps.every((step) => step.nodeIds.length > 0)).toBe(true);
  });
});
