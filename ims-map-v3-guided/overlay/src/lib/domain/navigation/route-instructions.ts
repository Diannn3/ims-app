...
function turnType(previous: GraphNode, current: GraphNode, next: GraphNode): 'turn-left' | 'turn-right' | null {
  ...
  // SVG/map coordinates grow downward on Y, so positive cross-product is a visual right turn.
...
export function buildRouteInstructions(
  ...
  const instructions: RouteInstruction[] = [
    {
      ...
      type: 'start',
      floor: nodes[0].floor,
      title: `Start at ${startLabel}`,
      detail: `Begin on the ${FLOOR_NAME[nodes[0].floor]}.`,
      nodeIds: [nodes[0].id]
    }
  ];
  ...
  const flushWalk = (endIndex: number) => {
    ...
    const metersLike = routeDistance(walkNodes);
    instructions.push({
      id: `walk-${sequence++}`,
      type: 'walk',
      floor: walkNodes[0].floor,
      title: endIndex === nodes.length - 1 ? `Continue toward ${destinationLabel}` : 'Continue along the corridor',
      detail: `Stay on the ${FLOOR_NAME[walkNodes[0].floor]}.`,
      nodeIds: walkNodes.map((node) => node.id),
      distance: Math.round(metersLike)
    });
  };
  ...
  for (let i = 1; i < nodes.length; i += 1) {
    ...
    if (previous.floor !== current.floor) {
      ...
      instructions.push({
        ...
        detail: 'Use the marked stair transition, then continue on the next floor.',
        nodeIds: [previous.id, current.id]
      });
      runStart = i;
      continue;
    }
    ...
    instructions.push({
      ...
      detail: `Continue on the ${FLOOR_NAME[current.floor]}.`,
      nodeIds: [previous.id, current.id, next.id]
    });
    runStart = i;
  }
  ...
  instructions.push({
    ...
    detail: `Your destination is on the ${FLOOR_NAME[last.floor]}.`,
    nodeIds: [last.id]
  });

  // Avoid adjacent duplicate walk instructions produced around a turn boundary.
